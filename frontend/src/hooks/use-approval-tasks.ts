...
import {
  ...
  bulkApproveApprovalTasks,
  bulkRejectApprovalTasks,
...
/**
 * Hook for bulk approving tasks
 */
export const useBulkApproveTasks = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await bulkApproveApprovalTasks(taskIds, comments, signature);
      if (!response.success) {
        throw new Error(response.error || 'Failed to bulk approve tasks');
      }
      return response.data;
    },
    onSuccess: (data, variables) => {
      const { successful, failed } = data || { successful: [], failed: [] };
      
      if (successful.length > 0) {
        toast.success(`Successfully approved ${successful.length} task(s)`);
      }
      
      if (failed.length > 0) {
        toast.error(`Failed to approve ${failed.length} task(s)`);
      }
    ...
    onError: (error: Error) => {
      toast.error(error.message || 'Failed to bulk approve tasks');
    },
  });
};

/**
 * Hook for bulk rejecting tasks
 */
export const useBulkRejectTasks = () => {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await bulkRejectApprovalTasks(taskIds, remarks, signature);
      if (!response.success) {
        throw new Error(response.error || 'Failed to bulk reject tasks');
      }
      return response.data;
    },
    onSuccess: (data, variables) => {
      ...
      if (successful.length > 0) {
        toast.success(`Successfully rejected ${successful.length} task(s)`);
      }
      
      if (failed.length > 0) {
        toast.error(`Failed to reject ${failed.length} task(s)`);
      }
    ...
    onError: (error: Error) => {
      toast.error(error.message || 'Failed to bulk reject tasks');
    },
  });
};
