...
export interface LinkedPVSummary {
  ...
  status: string;
  /** Amount of this specific payment voucher (present on the multi-PV `linkedPVs` list) */
  amount?: number;
}
...
export interface PurchaseOrder extends WorkflowDocument {
  ...
  /** Linked payment voucher summary (populated on list/detail responses). Kept for
   *  back-compat — this is the latest PV; prefer `linkedPVs` for the full list. */
  linkedPV?: LinkedPVSummary;
  /** All payment vouchers linked to this PO (multi-PV / partial-payment support) */
  linkedPVs?: LinkedPVSummary[];
  /** Derived payment status across all linked PVs */
  paymentStatus?: "unpaid" | "partially_paid" | "fully_paid";
  /** Total amount actually paid (from PVs marked paid) */
  amountPaid?: number;
  /** Total amount committed across all live (non-cancelled/rejected) PVs */
  amountCommitted?: number;
  /** Remaining balance available for new PVs (totalAmount - amountCommitted) */
  balance?: number;
