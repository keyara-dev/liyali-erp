...
export const getRequisitions = cache(async () => {
  try {
    ...
    try {
      localStorage.setItem('liyali_requisitions_cache', JSON.stringify(workflowDocs));
    } catch (error) {
...
liyali_requisitions_cache
liyali_purchase_orders_cache
liyali_payment_vouchers_cache

// Specific item keys
liyali_requisition_{id}_cache
liyali_purchase_order_{id}_cache
```
...
// localStorage
Object.keys(localStorage).filter(k => k.startsWith('liyali_'))
