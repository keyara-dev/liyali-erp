...
Invitee flow in `/invitations` page:
1. Card shown per pending invitation (org name, role, invited-by)
2. Accept → user added to org, redirected/refreshed
3. Decline → invitation marked declined

Admin flow in `/admin/invitations`:
- Table with status filter tabs (All / Pending / Accepted / Declined / Expired / Cancelled)
- Cancel / Resend actions per row
