...
-- Migration: 001_consolidated_complete_schema
-- Description: Complete database schema with all fixes and enhancements
-- Version: Consolidated from all previous migrations (001-006)
...
-- Organization Departments (must be created before organization_members due to foreign key dependency)
...
-- ============================================================================
-- MASTER DATA TABLES (WITH MULTI-TENANT FIXES)
-- ============================================================================

-- Organization-Scoped Vendors (FIXED: includes organization_id from migration 006)
...
-- ============================================================================
-- UNIFIED DOCUMENTS TABLE (FROM MIGRATION 005)
-- ============================================================================
...
-- ============================================================================
-- BUSINESS DOCUMENT TABLES (Enhanced with all fields)
-- ============================================================================
...
-- Purchase Orders (FIXED: vendor_id nullable from migration 004)
CREATE TABLE IF NOT EXISTS purchase_orders (
    ...
    CONSTRAINT fk_purchase_orders_vendor FOREIGN KEY (vendor_id) REFERENCES vendors(id) ON DELETE SET NULL, -- NULLABLE
    CONSTRAINT fk_purchase_orders_created_by FOREIGN KEY (created_by) REFERENCES users(id)
    -- CONSTRAINT fk_purchase_orders_source_requisition_id FOREIGN KEY (source_requisition_id) REFERENCES requisitions(id) ON DELETE SET NULL -- NULLABLE - TEMPORARILY DISABLED
);
...
CREATE INDEX IF NOT EXISTS idx_org_members_department_id ON organization_members(department_id);
...
CREATE INDEX IF NOT EXISTS idx_org_departments_active ON organization_departments(active);
CREATE INDEX IF NOT EXISTS idx_org_departments_is_active ON organization_departments(is_active);
CREATE INDEX IF NOT EXISTS idx_org_departments_parent_id ON organization_departments(parent_id);
...
-- Documents table indexes (from migration 005)
...
-- Create unique index for document numbers within organization
CREATE UNIQUE INDEX IF NOT EXISTS idx_documents_org_number ON documents(organization_id, document_number) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_documents_org_created_by ON documents(organization_id, created_by);

-- Vendors indexes (with organization_id from migration 006)
...
-- Add trigger to automatically update updated_at timestamp for documents
...
COMMENT ON COLUMN documents.document_number IS 'Unique document number within organization';
COMMENT ON COLUMN documents.data IS 'Document-specific data as JSONB';
COMMENT ON COLUMN documents.metadata IS 'Additional metadata and custom fields';

-- Vendors table comments
COMMENT ON COLUMN vendors.organization_id IS 'Organization ID for multi-tenant isolation';
...
DO $$
BEGIN
    RAISE NOTICE 'Migration 001_consolidated_complete_schema completed successfully';
    ...
    RAISE NOTICE '✅ Standardized organization tiers (starter/pro/enterprise)';
