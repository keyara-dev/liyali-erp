-- Migration: 012_subscription_management_system_pg
-- Description: Create comprehensive subscription management system for admin console (PostgreSQL version)
-- Date: 2024-02-05
...
CREATE TABLE IF NOT EXISTS subscription_tiers (
    ...
    features JSONB NOT NULL DEFAULT '[]', -- JSON array of feature names
...
CREATE TABLE IF NOT EXISTS organization_limit_overrides (
    ...
    features JSONB, -- JSON array of additional features
...
CREATE TABLE IF NOT EXISTS admin_audit_logs (
    ...
    action VARCHAR(255) NOT NULL, -- 'tier_change', 'limit_override', 'trial_reset', etc.
    old_value TEXT,
    new_value TEXT,
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='organizations' AND column_name='subscription_tier') THEN
        ALTER TABLE organizations ADD COLUMN subscription_tier VARCHAR(255) DEFAULT 'basic';
    END IF;
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='organizations' AND column_name='trial_start_date') THEN
        ALTER TABLE organizations ADD COLUMN trial_start_date TIMESTAMP WITH TIME ZONE;
    END IF;
...
INSERT INTO subscription_tiers (id, name, display_name, description, price_monthly, price_yearly, max_users, storage_limit_gb, features, is_active, sort_order) VALUES
