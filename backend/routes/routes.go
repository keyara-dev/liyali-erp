...
func SetupRoutes(app *fiber.App, handlerRegistry *handlers.HandlerRegistry, rbacService *services.RBACService, db *gorm.DB, activityService ...*services.ActivityService) {
	...
	grns.Post("/:id/complete", middleware.RequirePermission(rbacService, "grn", "edit"), handlers.MarkGRNComplete)
	...
	adminAPIMonitoring := admin.Group("/api-monitoring")
	adminAPIMonitoring.Get("/stats", handlers.GetAPIStats)
	adminAPIMonitoring.Get("/performance", handlers.GetAPIPerformance)
	adminAPIMonitoring.Get("/categories", handlers.GetAPICategories)
	adminAPIMonitoring.Get("/realtime", handlers.GetAPIRealtimeMetrics)
	adminAPIMonitoring.Post("/export", handlers.ExportAPIMonitoringData)
	adminAPIMonitoring.Post("/alert-rules", handlers.CreateAPIAlertRule)
	adminAPIMonitoring.Get("/endpoints", handlers.GetAPIEndpoints)
	adminAPIMonitoring.Get("/metrics", handlers.GetAPIMetrics)
	adminAPIMonitoring.Get("/errors", handlers.GetAPIErrors)
	adminAPIMonitoring.Get("/alerts", handlers.GetAPIAlerts)
	adminAPIMonitoring.Get("/endpoints/:id", handlers.GetAPIEndpointByID)
	adminAPIMonitoring.Get("/endpoints/:id/metrics", handlers.GetAPIEndpointMetrics)
	adminAPIMonitoring.Post("/endpoints/:id/test", handlers.TestAPIEndpoint)
	adminAPIMonitoring.Put("/endpoints/:id/config", handlers.UpdateAPIEndpointConfig)
	adminAPIMonitoring.Get("/errors/:id", handlers.GetAPIErrorByID)
	adminAPIMonitoring.Post("/errors/:id/resolve", handlers.ResolveAPIError)
	adminAPIMonitoring.Post("/alerts/:id/acknowledge", handlers.AcknowledgeAPIAlert)
	adminAPIMonitoring.Post("/alerts/:id/resolve", handlers.ResolveAPIAlert)
