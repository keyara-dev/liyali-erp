# Liyali Gateway - Go Fiber Backend

**Phase 12 Implementation** - PostgreSQL Database Integration

## Overview

This is the Go Fiber backend for the Liyali Gateway workflow approval system. It replaces the localStorage-based frontend implementation with a production-ready PostgreSQL database and REST API.

### Architecture

```
Backend (Go Fiber)
├── PostgreSQL Database (liyali-dev-db)
├── GORM ORM Layer
...
# Or using psql
psql -U postgres -c "CREATE DATABASE \"liyali-dev-db\";"
```
...
```bash
go build -o liyali-gateway-backend ./main.go
```
